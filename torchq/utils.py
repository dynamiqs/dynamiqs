...
def from_qt(x: qt.Qobj) -> torch.tensor:
...
def to_qt(x: torch.Tensor, dims: List[Any]) -> qt.Qobj:
    ...
    """
    return qt.Qobj(to_np(x), dims=dims)


def to_torch(x: Union[qt.Qobj, np.ndarray, List[Any]]) -> torch.tensor:
    """Convert a QuTiP quantum object or NumPy array or Python list to a
    PyTorch tensor.
    ...
    Args:
        x: QuTiP quantum object or NumPy array or Python list.
    ...
    if isinstance(x, qt.Qobj):
        ...
        raise TypeError(
            'invalid type (supported: QuTiP quantum object, NumPy array or'
            'Python list)'
        )
