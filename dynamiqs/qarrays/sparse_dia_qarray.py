...
def to_dense(x: SparseDIAQArray) -> DenseQArray:
    ...
    for o, diag in zip(x.offsets, diags):
        s = max(0, o)
        e = min(N, N + o)
        tensor = jnp.stack([jnp.diag(diag[i, s:e], k=o) for i in range(d)], axis=0)
...
def stack(x_list: list[SparseDIAQArray]) -> SparseDIAQArray:
    diags = jnp.stack([x.diags for x in x_list], axis=0)
    offsets = x_list[0].offsets
    return SparseDIAQArray(x_list[0].dims, offsets, diags)
