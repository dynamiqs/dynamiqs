...
class IterativeSolver(AbstractSolver):
    ...
    def run(self) -> Result:
        ...
        # todo: Should we use jax.lax.scan here? It's not clear to me how to accumulate
        #       the results directly in the sub-arrays of the `result` PyTree, the
        #       default jax.lax.scan returns a tuple of PyTrees.
...
class StatelessIterativeSolver(IterativeSolver):
    ...
    def step(self, t0: Scalar, t1: Scalar, solver_state: SolverState) -> SolverState:
        # todo: Check if the named tuple method _replace() is JIT-compatible.
