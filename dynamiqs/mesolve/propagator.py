...
class MEPropagator(MESolver, Propagator):
    def __init__(self, *args, **kwargs):
        ...
        self.lindbladian = slindbladian(self.H, self.L)  # (b_H, b_L, 1, n^2, n^2)
        self.y0 = operator_to_vector(self.y0)  # (b_H, b_L, b_rho, n^2, 1)
    ...
    def forward(self, t: float, delta_t: float, rho_vec: Tensor) -> Tensor:
        # rho: (b_H, b_L, b_rho, n^2, 1) -> (b_H, b_L, b_rho, n^2, 1)
        return self.propagator(delta_t) @ rho_vec  # (b_H, b_L, b_rho, n^2, 1)

    def save(self, y: Tensor):
        # override `save` method to convert `y` from vector to operator
        y = vector_to_operator(y)  # (b_H, b_L, b_rho, n, n)
        super().save(y)
