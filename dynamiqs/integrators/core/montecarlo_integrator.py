...
class MESolveDiffusiveMonteCarloIntegrator(BaseIntegrator, MEInterface, SolveInterface):
    def __post_init__(self):
        # check that y0 is a ket
        if not self.y0.isket():
            raise ValueError(
                'For the `DiffusiveMonteCarlo` method, `rho0` must be a ket, '
                f'but has shape {self.y0.shape}.'
            )
    ...
    def run(self) -> Result:
        ...
        dsse_result = _vectorized_dssesolve(
            self.H,
            self.Ls,
            self.y0,
            self.ts,
        ...
        # compute mean states and expectation values
        ysave = dsse_result.mean_states()
        Esave = dsse_result.mean_expects()
        ...
        if self.options.save_extra:
            if self.options.save_states:
                ...
                # be equal to mean(save_extra(states)).
                warnings.warn(
                    'The `save_extra` option is not supported with the '
                    '`DiffusiveMonteCarlo` method unless the `save_states` option is '
                    'also set to True. Falling back to extra=None.',
                    stacklevel=2,
                )

        # return
        saved = SolveSaved(ysave=ysave, extra=extra, Esave=Esave)
        return self.result(saved, infos=dsse_result.infos)
