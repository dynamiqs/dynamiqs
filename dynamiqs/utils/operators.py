...
@dispatch_matrix_format
def eye(*dims: int, matrix_format: MatrixFormatType = None) -> QArray:  # noqa: ARG001
...
@dispatch_matrix_format
def zero(*dims: int, matrix_format: MatrixFormatType = None) -> QArray:  # noqa: ARG001
...
@register_format_handler('zero', MatrixFormat.DENSE)
...
@register_format_handler('zero', MatrixFormat.SPARSE_DIA)
...
@dispatch_matrix_format
def destroy(
    *dims: int, matrix_format: MatrixFormatType = None  # noqa: ARG001
) -> QArray | tuple[QArray, ...]:
...
@register_format_handler('destroy', MatrixFormat.DENSE)
def destroy_dense(*dims: int) -> QArray | tuple[QArray, ...]:
    ...
    Id = [eye(dim, matrix_format=MatrixFormat.DENSE) for dim in dims]
    return tuple(
...
@register_format_handler('destroy', MatrixFormat.SPARSE_DIA)
def destroy_sparse_dia(*dims: int) -> QArray | tuple[QArray, ...]:
    ...
    Id = [eye(dim, matrix_format=MatrixFormat.SPARSE_DIA) for dim in dims]
...
@register_format_handler('create', MatrixFormat.DENSE)
...
@register_format_handler('create', MatrixFormat.SPARSE_DIA)
...
@dispatch_matrix_format
def number(dim: int | None = None, *, matrix_format: MatrixFormatType = None) -> QArray:
...
@dispatch_matrix_format
def parity(dim: int, *, matrix_format: MatrixFormatType = None) -> QArray:
...
@register_format_handler('parity', MatrixFormat.DENSE)
...
@register_format_handler('parity', MatrixFormat.SPARSE_DIA)
...
def displace(
    ...
    a = destroy(dim, matrix_format=MatrixFormat.DENSE)  # (n, n)
...
def quadrature(
    dim: int, phi: float, *, matrix_format: MatrixFormatType = None
) -> QArray:
...
def position(dim: int, *, matrix_format: MatrixFormatType = None) -> QArray:
...
def momentum(dim: int, *, matrix_format: MatrixFormatType = None) -> QArray:
...
@dispatch_matrix_format
def sigmax(*, matrix_format: MatrixFormatType = None) -> QArray:  # noqa: ARG001
...
@register_format_handler('sigmax', MatrixFormat.DENSE)
def sigmax_dense() -> QArray:
    array = jnp.array([[0.0, 1.0], [1.0, 0.0]], dtype=cdtype())
    return asqarray(array)


@register_format_handler('sigmax', MatrixFormat.SPARSE_DIA)
...
@dispatch_matrix_format
def sigmay(*, matrix_format: MatrixFormatType = None) -> QArray:  # noqa: ARG001
...
@register_format_handler('sigmay', MatrixFormat.DENSE)
def sigmay_dense() -> QArray:
    array = jnp.array([[0.0, -1.0j], [1.0j, 0.0]], dtype=cdtype())
    return asqarray(array)


@register_format_handler('sigmay', MatrixFormat.SPARSE_DIA)
...
@dispatch_matrix_format
def sigmaz(*, matrix_format: MatrixFormatType = None) -> QArray:  # noqa: ARG001
...
@register_format_handler('sigmaz', MatrixFormat.DENSE)
def sigmaz_dense() -> QArray:
    array = jnp.array([[1.0, 0.0], [0.0, -1.0]], dtype=cdtype())
    return asqarray(array)


@register_format_handler('sigmaz', MatrixFormat.SPARSE_DIA)
...
@dispatch_matrix_format
def sigmap(*, matrix_format: MatrixFormatType = None) -> QArray:  # noqa: ARG001
...
@register_format_handler('sigmap', MatrixFormat.DENSE)
def sigmap_dense() -> QArray:
    array = jnp.array([[0.0, 1.0], [0.0, 0.0]], dtype=cdtype())
    return asqarray(array)


@register_format_handler('sigmap', MatrixFormat.SPARSE_DIA)
...
@dispatch_matrix_format
def sigmam(*, matrix_format: MatrixFormatType = None) -> QArray:  # noqa: ARG001
...
@register_format_handler('sigmam', MatrixFormat.DENSE)
def sigmam_dense() -> QArray:
    array = jnp.array([[0.0, 0.0], [1.0, 0.0]], dtype=cdtype())
    return asqarray(array)


@register_format_handler('sigmam', MatrixFormat.SPARSE_DIA)
...
@dispatch_matrix_format
def hadamard(n: int = 1, *, matrix_format: MatrixFormatType = None) -> QArray:
...
@register_format_handler('hadamard', MatrixFormat.DENSE)
...
@register_format_handler('hadamard', MatrixFormat.SPARSE_DIA)
