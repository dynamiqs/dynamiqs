...
class ODEIntAdjoint(torch.autograd.Function):
    ...
    def forward(ctx, f, f_adj, y0, tspan, solver, save_at, atol, rtol, *params):
        ...
        with torch.no_grad():
            t, y = grt.odeint(f, y0, tspan, solver, save_at, atol=atol, rtol=rtol)
    ...
    def backward(ctx, *grad_y):
        ...
        params_indices = torch.cat([torch.tensor([0]), torch.cumsum(params_numels, dim=0)])
        
        # Initialize the augmented state as a flat tensor
        # y is the state, a is the adjoint, g is the gradient w.r.t. parameters
        yT, aT, gT = y_saved[-1], grad_y[-1][-1], torch.zeros(n_params, dtype=y_saved.dtype)
        ...
        if solver_type == "rk":
            ...
            def aug_f(t, aug_y):
                ...
                with torch.enable_grad():
                    ...
                    dg = torch.autograd.grad(da, params, y.view(-1), 
                                            allow_unused=True, retain_graph=True)
                    # Convert back dg to flat tensor
                    dg = torch.cat([torch.zeros_like(p).view(-1) if dg_ is None else dg_.view(-1)
                                for p, dg_ in zip(params, dg)]).conj()
        ...
        for i in range(len(t_saved) - 1, 0, -1):
            ...
            _, aug_y = grt.odeint(aug_f, aug_y, tspan_, solver=solver, save_at=(),
        ...
        grads_out = tuple(g[params_indices[i]:params_indices[i+1]].reshape(params_shapes[i])
