...
def test_odeint(prob, solver, dtype, test_iter):
    ...
    # Compute solutions
    yt_exp = p.solution()
    t, yt = odeint(p, p.y0, p.tspan, save_at=p.tspan, solver=solver, atol=p.atol, rtol=p.rtol)
...
def test_odeint_adjoint(prob, solver, dtype, test_iter):
    ...
    # Compute solutions
    yt_exp = p.solution()
    t, yt = odeint_adjoint(p, p.y0, p.tspan, save_at=p.tspan, solver=solver, atol=p.atol, rtol=p.rtol)
    _, yt_tdyn = odeint_tdyn(p, p.y0, p.tspan, solver, atol=p.atol, rtol=p.rtol)
    # Check solutions
    assert torch.norm((yt - yt_exp) / yt_exp) < p.test_tol
    assert torch.norm((yt - yt_tdyn) / yt_tdyn) < p.test_tol
...
def test_odeint_backward(prob, solver, dtype, test_iter):
    ...
    # Compute solutions
    yt_exp = p.solution()
    t, yt = odeint(p, p.y0, p.tspan, save_at=p.tspan, solver=solver, 
                   atol=p.atol, rtol=p.rtol, backward_mode=False)
...
def test_odeint_out(prob, solver, dtype, test_iter):
    ...
    # Compute solutions
    yt_exp = p.solution()
    t, yt = odeint(p, p.y0, p.tspan, save_at=p.save_at, solver=solver, atol=p.atol, rtol=p.rtol)
    _, yt_tdyn = odeint_tdyn(p_ref, p_ref.y0, p_ref.tspan, 'dopri5', atol=p_ref.atol, rtol=p_ref.rtol)
    # Check solutions
    assert torch.norm((yt - yt_exp) / yt_exp) < p.test_tol
    assert torch.norm((yt - yt_tdyn) / yt_tdyn) < p.test_tol
    # Check output dtype
...
def test_odeint_out_adjoint(prob, solver, dtype, test_iter):
    ...
    # Compute solutions
    yt_exp = p.solution()
    t, yt = odeint_adjoint(p, p.y0, p.tspan, save_at=p.save_at, solver=solver, atol=p.atol, rtol=p.rtol)
    _, yt_tdyn = odeint_tdyn(p_ref, p_ref.y0, p_ref.tspan, 'dopri5', atol=p_ref.atol, rtol=p_ref.rtol)
    # Check solutions
    assert torch.norm((yt - yt_exp) / yt_exp) < p.test_tol
    assert torch.norm((yt - yt_tdyn) / yt_tdyn) < p.test_tol
...
def test_odeint_out_backward(prob, solver, dtype, test_iter):
    ...
    # Compute solutions
    yt_exp = p.solution()
    t, yt = odeint(p, p.y0, p.tspan, save_at=p.save_at, solver=solver, 
                   atol=p.atol, rtol=p.rtol, backward_mode=False)
